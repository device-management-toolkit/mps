...
describe('Boot Options', () => {
  ...
  beforeEach(() => {
    ...
    getSetupAndConfigurationServiceSpy.mockResolvedValue({
      Body: { AMT_SetupAndConfigurationService: { ProvisioningMode: 4 } } // Default to ACM mode
    })
  ...
  it('should return 400 error when EnforceSecureBoot is false in CCM mode', async () => {
    getSetupAndConfigurationServiceSpy.mockResolvedValue({
      Body: { AMT_SetupAndConfigurationService: { ProvisioningMode: 1 } } // CCM mode
    })
    req.body = {
      action: 105,
      useSOL: false,
  ...
  it('should return 400 error when EnforceSecureBoot is false in CCM mode (string ProvisioningMode)', async () => {
    getSetupAndConfigurationServiceSpy.mockResolvedValue({
      Body: { AMT_SetupAndConfigurationService: { ProvisioningMode: '1' } } // CCM mode as string
    })
    req.body = {
      action: 105,
      useSOL: false,
  ...
  it('should succeed when EnforceSecureBoot is false in ACM mode', async () => {
    getSetupAndConfigurationServiceSpy.mockResolvedValue({
      Body: { AMT_SetupAndConfigurationService: { ProvisioningMode: 4 } } // ACM mode
    })
    req.body = {
      action: 105,
      useSOL: false,
  ...
  it('should succeed when bootDetails is not provided in CCM mode', async () => {
    getSetupAndConfigurationServiceSpy.mockResolvedValue({
      Body: { AMT_SetupAndConfigurationService: { ProvisioningMode: 1 } } // CCM mode
    })
    req.body = {
