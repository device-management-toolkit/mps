...
export async function patchWiredNetworkSettings(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (putResponse == null) {
      logger.error(`${messages.WIRED_NETWORK_SETTINGS_UPDATE_FAILED} for guid : ${guid}.`)
      MqttProvider.publishEvent(
        'fail',
        ['AMT_EthernetPortSettings'],
      ...
      res.status(400).json(ErrorResponse(400, `${messages.WIRED_NETWORK_SETTINGS_UPDATE_FAILED} for guid : ${guid}.`))
      return
    }
