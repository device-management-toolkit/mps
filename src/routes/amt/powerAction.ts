...
async function handleOSPowerSavingStateChange(req: Request, res: Response, action: number): Promise<void> {
  ...
  if (returnValue === 0) {
    logger.info(`OS Power Saving State changed to ${targetStateValue} (${stateName}) successfully.`)
    sendOSPowerResponse(res, 0, OSPowerSavingStateStatusCodesToString(0))
  } else {
    logger.error(`Failed to change OS Power Saving State (Target: ${stateName}).`)
