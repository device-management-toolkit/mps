...
import { TimeoutError } from '../utils/timeoutOpManagement.js'
...
describe('PowerStateRefresher', () => {
  ...
  it('should timestamp the reading when the device read starts', async () => {
    let release: (value: { powerState: number; osPowerSavingState: number }) => void
    const readSpy = vi.spyOn(refresher, 'read').mockReturnValue(
      new Promise((resolve) => {
        release = resolve
      })
    )
    ...
    release({ powerState: 4, osPowerSavingState: 2 })
    await pending

    const updatedAt = updatePowerStateSpy.mock.calls[0][3] as Date
    expect(updatedAt.getTime()).toBeLessThan(completedAfter)
  })
  ...
  it('should suspend refresh until reconnect after a timeout', async () => {
    const readSpy = vi.spyOn(refresher, 'read').mockRejectedValueOnce(new TimeoutError('Timeout'))
    refresher.onConnect(guid)

    expect(await refresher.maybeRefresh(guid, device)).toBe(false)
    expect(refresher.state.get(guid).nextEligible).toBe(Number.POSITIVE_INFINITY)
    expect(await refresher.maybeRefresh(guid, device)).toBe(false)
    expect(readSpy).toHaveBeenCalledTimes(1)

    refresher.onDisconnect(guid)
    refresher.onConnect(guid)
    readSpy.mockResolvedValue({ powerState: 4, osPowerSavingState: 0 })
  ...
  describe('read', () => {
    ...
    it('should propagate an OS power saving state timeout', async () => {
      wsman.getPowerState.mockResolvedValue(pullResponse('4'))
      wsman.getOSPowerSavingState.mockRejectedValue(new TimeoutError('Timeout'))

      await expect(refresher.read(device)).rejects.toBeInstanceOf(TimeoutError)
    })
