...
describe('device tests', () => {
  ...
  test('should get true when power state is updated', async () => {
    ...
    expect(querySpy).toHaveBeenCalledWith(
      ...
      WHERE guid=$1 and tenantid = $5
        AND (powerstateupdatedat IS NULL OR powerstateupdatedat <= $4)`,
      [
        '4c4c4544-004b-4210-8033-b6c04f504633',
        4,
        2,
  ...
  test('should return false without retrying when a newer cached reading prevents the update', async () => {
    querySpy.mockResolvedValueOnce({ rows: [], command: '', fields: null, rowCount: 0, oid: 0 })
    const updatedAt = new Date('2026-08-25T17:00:00.000Z')

    expect(await deviceTable.updatePowerState('4c4c4544-004b-4210-8033-b6c04f504633', 4, 2, updatedAt)).toBe(false)
    expect(querySpy).toHaveBeenCalledTimes(1)
    expect(querySpy).toHaveBeenCalledWith(
      expect.stringContaining('AND (powerstateupdatedat IS NULL OR powerstateupdatedat <= $4)'),
      [
        '4c4c4544-004b-4210-8033-b6c04f504633',
        4,
        2,
