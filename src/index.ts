...
export async function setupServiceManager(config: configType): Promise<void> {
  if (config.consul_enabled) {
    ...
    try {
      ...
      // Consul replaces Environment.Config with its stored JSON. Merge in any
      // newly introduced defaults, then validate the effective remote values.
      Environment.Config = loadConfig({ ...config, ...Environment.Config })
    } catch (err) {
...
export function loadConfig(config: any): configType {
  ...
  if (config.mps_tls_config.mps_cert_key_size != null) {
    ...
    if (!ALLOWED_MPS_CERT_KEY_SIZES.includes(certKeySize)) {
      logger.warn(
        `Invalid mps_tls_config.mps_cert_key_size "${config.mps_tls_config.mps_cert_key_size}", using default ${DEFAULT_MPS_CERT_KEY_SIZE}. Allowed values: ${ALLOWED_MPS_CERT_KEY_SIZES.join(', ')}`
      )
